...
public class ShipPreTest {
	...
	public void testUpdateResults2() {
		ship.updateResults(-1);
		assertEquals(ship.getLosses(),1,ship.getLosses());
		assertEquals(0,ship.getWins());
	}
	 ...
	 * del tipo 'XWing' excepto el ultimo (se usa el metodo auxiliar destroy(String, int) 
	 * para ello) y se intenta obtener el primer caza no destruido de la nave. 
	 * Se comprueba que devuelve exactamente el ultimo de los 'XWing'. 
	 */
	@Test
	public void testGetFirstAvailableFighter6() {
		...
		assertNotNull(fighter);
		/*
		 * comprueba que fighter es un XWing y es el que ocupa la posicion 21 de la flota
		 */
	...
	public void testMyFleet2() {
		...
		ship.addFighters(kFleet2);
		assertEquals(ship.myFleet(),kFleet22, ship.myFleet());	
	}
